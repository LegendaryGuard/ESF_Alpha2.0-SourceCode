...
void CBasePlayer :: Pain( void )
{
	...
	/*char pain_dir[255];
	sprintf (pain_dir,"%s/pain1.wav",m_pClass->GetName());
	EMIT_SOUND(ENT(pev), CHAN_VOICE, pain_dir, 1, ATTN_NORM);*/
	if ( flRndSound <= 0.33 )
...
{	int n;
	...
	case PLAYER_WALK:
		...
		else if ( !FBitSet( pev->flags, FL_ONGROUND ) && (m_Activity == ACT_HOP || m_Activity == ACT_LEAP) && !m_fFlying)	// Still jumping 
		{	m_IdealActivity = m_Activity;
		}
	...
	case ACT_WALK:
		...
		/* modified by harSens
		if ( (m_Activity != ACT_RANGE_ATTACK1 && m_Activity !=ACT_PUNCH ) || m_fSequenceFinished)
		*/
...
void CBasePlayer :: UpdateStepSound( void )
{
	...
	{	//changed by harSens
		...
		velwalk = GetMaxSpeed()/4;	// These constants should be based on cl_movespeedkey * cl_forwardspeed somehow
		velrun = GetMaxSpeed()/3;	// UNDONE: Move walking to server
		flduck = 0.1;
	}
	else
	{	//changed by harSens
...
{	pev->classname		= MAKE_STRING("player");
	...
	*/
	...
	m_iClientSpeed = 0; // force speed update to client 
 	g_engfuncs.pfnSetClientMaxspeed( ENT( pev ), GetMaxSpeed() ); 
	//end harSens add
...
/* harSens: moved upward, needed to cover cell code :)
extern float g_flWeaponCheat;
*/
